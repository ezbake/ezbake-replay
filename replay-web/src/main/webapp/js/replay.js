...
$(document).ready(function() {
    ...
    function populateStatusTable(data) {
        ...
        if (data && data.replayHistory) {
            html = "<table class='formatHTML5'><thead><tr><td>Submit Time</td><td>Start</td><td>Finish</td><td>Uri</td><td>Topic</td><td>Total</td><td>Count</td><td>Status</td><td>Last Broadcast Version</td><td>Remove</td></tr></thead>";

            for (var key in data.replayHistory) {
                var entry = data.replayHistory[key];
                var submitTime = moment(new Date(+key)).format('HH:mm:ss MM/DD/YYYY Z');
                html += "<tr><td>" + submitTime + "</td>";
